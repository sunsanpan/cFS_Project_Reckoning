#!/usr/bin/env python3
"""
FSWV1 cFS App - Command Test Script

This script sends telecommands to the FSWV1 cFS application for testing.
It can send individual commands or run automated test sequences.

Usage:
    python3 fswv1_cmd_test.py --help
    python3 fswv1_cmd_test.py --led-on
    python3 fswv1_cmd_test.py --test-all
    python3 fswv1_cmd_test.py --blink 5
...
# Message IDs (must match your fswv1_app_msgids.h)
FSWV1_APP_CMD_MID = 0x1884

# Command Codes (from fswv1_app_msg.h)
CMD_NOOP = 0
...
CCSDS_SEC_HDR = 1
CCSDS_APID = FSWV1_APP_CMD_MID & 0x7FF
...
class CFSCommand:
    ...
    def send_command(self, cmd_code, data=b'', verbose=True):
        ...
        header = self.build_ccsds_header(FSWV1_APP_CMD_MID, cmd_code, len(data))
        message = header + data
...
def interactive_menu(cfs):
    ...
    while True:
        print("\n" + "="*60)
        print("FSWV1 cFS Command Test - Interactive Mode")
        print("="*60)
        print("\nBasic Commands:")
        print("  0. NOOP")
...
def main():
    parser = argparse.ArgumentParser(
        description='Send commands to FSWV1 cFS application',
...
Examples:
  ...
  %(prog)s --host 192.168.1.100      # Connect to remote cFS
    ...
    print("="*60)
    print("FSWV1 cFS Command Test Script")
    print("="*60)
    print(f"Target: {args.host}:{args.port}")
    print(f"Message ID: 0x{FSWV1_APP_CMD_MID:04X}")
    print("="*60)
